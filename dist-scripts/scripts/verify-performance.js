...
function benchmarkEpgWindowing() {
    ...
    node_assert_1.default.ok(avgTickMs < 5.0, `EPG average scroll tick (${avgTickMs}ms) must be < 5ms`);
...
function benchmarkGlobalSearch() {
    ...
    const generateStreams = (count, prefix) => {
    ...
    node_assert_1.default.ok(maxSearchMs < 2000, `Global search max latency (${maxSearchMs}ms) must be < 2000ms (2s)`);
